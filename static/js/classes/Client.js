...
export default class Client {
  ...
  static async init(nick) {
    ...
    if (res.ok) {
      ...
      sessionStorage.setItem('client_status', resData.status);
  ...
  static async setStatus(status) {
    console.log(`${Utils.fullTime(new Date())} [WORKING] Setting client status.`);
    ...
    let res = await fetch('/join/status', {
      body: JSON.stringify(reqData),
      method: 'post'
    });

    if (res.ok)
      console.log(`${Utils.fullTime(new Date())} [SUCCESS] Set client status.`);
    else
      console.log(`${Utils.fullTime(new Date())} [ERROR] Failed set client status.`);
  }
